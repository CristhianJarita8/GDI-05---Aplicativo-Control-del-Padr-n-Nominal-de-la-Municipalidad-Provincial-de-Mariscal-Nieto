...
from db.services.apoderado_service import ApoderadoService
from ui.theme import IconButton


class ApoderadoForm(tk.Toplevel):
    def __init__(self, parent):
        super().__init__(parent)
        self.title("Apoderado")
        self.geometry("900x520")
        self.service = ApoderadoService()

        labels = [
            ("DNI_Apoderado",0),("DNI_CE",1),("Codigo_PadronNominal",2),("Nombres_Apoderado",3),
            ("Apellidos_Apoderado",4),("GradoInstruccion_Madre",5),("Telefono",6)
        ]
        self.vars = {}
        ...
        cols = ["DNI_Apoderado","DNI_CE","Codigo_PadronNominal","Nombres_Apoderado","Apellidos_Apoderado","GradoInstruccion_Madre","Telefono"]
    ...
    def _read_form(self):
        return (
            self.vars["DNI_Apoderado"].get().strip(),
            self.vars["DNI_CE"].get().strip(),
            self.vars["Codigo_PadronNominal"].get().strip(),
            self.vars["Nombres_Apoderado"].get().strip(),
            self.vars["Apellidos_Apoderado"].get().strip(),
            self.vars["GradoInstruccion_Madre"].get().strip(),
    ...
    def insertar(self):
        ...
        try:
            ...
            messagebox.showinfo("Éxito", "Apoderado insertado.")
            self.refrescar_lista()
            self._clear_form()
    ...
    def cargar_para_modificar(self):
        dlg = SelectDialog(self, ["DNI_Apoderado","DNI_CE"])
        self.wait_window(dlg)
        if not getattr(dlg, "values", None):
            return
        pk = dlg.values
        try:
            row = self.service.get_by_pk(pk["DNI_Apoderado"], pk["DNI_CE"])
            if not row:
                messagebox.showinfo("No encontrado", "No hay registro con esas claves.")
                return
            self.vars["DNI_Apoderado"].set(row.get("DNI_Apoderado") or "")
            self.vars["DNI_CE"].set(row.get("DNI_CE") or "")
            self.vars["Codigo_PadronNominal"].set(row.get("Codigo_PadronNominal") or "")
            self.vars["Nombres_Apoderado"].set(row.get("Nombres_Apoderado") or "")
            self.vars["Apellidos_Apoderado"].set(row.get("Apellidos_Apoderado") or "")
            self.vars["GradoInstruccion_Madre"].set(row.get("GradoInstruccion_Madre") or "")
    ...
    def actualizar(self):
        ...
        if not (data[0] and data[1]):
            messagebox.showwarning("PK faltante", "DNI_Apoderado y DNI_CE son obligatorios para actualizar.")
            return
        try:
            self.service.modificar(data)
            messagebox.showinfo("Éxito", "Apoderado actualizado.")
            self.refrescar_lista()
            self._clear_form()
        except Exception as e:
    ...
    def eliminar(self):
        ...
        if not getattr(dlg, "values", None):
            return
        pk = dlg.values
        confirm = ConfirmDialog(self, f"¿Eliminar Apoderado {pk}?")
        self.wait_window(confirm)
        if not getattr(confirm, "response", False):
            return
        try:
            self.service.eliminar(pk["DNI_Apoderado"], pk["DNI_CE"])
