...
from db.services.representanteMunicipal_service import RepresentanteMunicipalService
from ui.theme import IconButton


class RepresentanteMunicipalForm(tk.Toplevel):
    def __init__(self, parent):
        super().__init__(parent)
        self.title("Representante_Municipal")
        self.geometry("900x520")
        self.service = RepresentanteMunicipalService()

        labels = [("DNI_RM",0),("ID_HomologacionNominal",1),("Nombre_RM",2)]
        self.vars = {}
        ...
        cols = ["DNI_RM","ID_HomologacionNominal","Nombre_RM"]
    ...
    def _read_form(self):
        return (
            self.vars["DNI_RM"].get().strip(),
            self.vars["ID_HomologacionNominal"].get().strip(),
            self.vars["Nombre_RM"].get().strip()
        )
    ...
    def insertar(self):
        ...
        try:
            ...
            messagebox.showinfo("Éxito", "Representante_Municipal insertado.")
            self.refrescar_lista()
            self._clear_form()
    ...
    def cargar_para_modificar(self):
        dlg = SelectDialog(self, ["DNI_RM","ID_HomologacionNominal"])
        self.wait_window(dlg)
        if not getattr(dlg, "values", None):
            return
        pk = dlg.values
        try:
            row = self.service.get_by_pk(pk["DNI_RM"], pk["ID_HomologacionNominal"])
            if not row:
                messagebox.showinfo("No encontrado", "No hay registro con esas claves.")
                return
            self.vars["DNI_RM"].set(row.get("DNI_RM") or "")
            self.vars["ID_HomologacionNominal"].set(row.get("ID_HomologacionNominal") or "")
            self.vars["Nombre_RM"].set(row.get("Nombre_RM") or "")
        except Exception as e:
    ...
    def actualizar(self):
        ...
        if not (data[0] and data[1]):
            messagebox.showwarning("PK faltante", "DNI_RM y ID_HomologacionNominal son obligatorios para actualizar.")
            return
        try:
            self.service.modificar(data)
            messagebox.showinfo("Éxito", "Representante_Municipal actualizado.")
            self.refrescar_lista()
            self._clear_form()
        except Exception as e:
    ...
    def eliminar(self):
        ...
        if not getattr(dlg, "values", None):
            return
        pk = dlg.values
        confirm = ConfirmDialog(self, f"¿Eliminar Representante_Municipal {pk}?")
        self.wait_window(confirm)
        if not getattr(confirm, "response", False):
            return
        try:
            self.service.eliminar(pk["DNI_RM"], pk["ID_HomologacionNominal"])
